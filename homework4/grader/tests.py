...
class ExtractPeakGrader(Grader):
    ...
    def test_radius1(self, min_score=0):
        ...
        for _min_score in np.linspace(min_s, max_s, 5):
            """min_score tests"""
            p = self.module.extract_peak(img, max_pool_ks=1, min_score=_min_score, max_det=100000)
            assert len(p) == (img > _min_score).sum(), 'Expected exactly %d detections, got %d' % ((img > _min_score).sum(), len(p))
            self.test_det(p, img, min_score=_min_score)
